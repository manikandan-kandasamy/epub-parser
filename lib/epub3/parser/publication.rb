...
module EPUB3
  class Parser
    class Publication
      ...
      def parse
        ([:package] + EPUB3::Publication::Package::CONTENT_MODELS).each do |model|
      ...
      def parse_package
        ...
        EPUB3::Publication.__send__ :include, EPUB3::Publication::FixedLayout if @package.prefix.key? EPUB3::Publication::FixedLayout::PREFIX_KEY

        @package
      end

      def parse_metadata
        metadata = @package.metadata = EPUB3::Publication::Package::Metadata.new
        elem = @doc.xpath('/opf:package/opf:metadata', EPUB3::NAMESPACES).first
        id_map = {}
      ...
      def parse_manifest
        manifest = @package.manifest = EPUB3::Publication::Package::Manifest.new
        elem = @doc.xpath('/opf:package/opf:manifest', EPUB3::NAMESPACES).first
        manifest.id = extract_attribute(elem, 'id')

        fallback_map = {}
        elem.xpath('./opf:item', EPUB3::NAMESPACES).each do |e|
      ...
      def parse_spine
        spine = @package.spine = EPUB3::Publication::Package::Spine.new
        elem = @doc.xpath('/opf:package/opf:spine', EPUB3::NAMESPACES).first
        %w[id toc page-progression-direction].each do |attr|
          spine.__send__ "#{attr.gsub(/-/, '_')}=", extract_attribute(elem, attr)
        end

        elem.xpath('./opf:itemref', EPUB3::NAMESPACES).each do |e|
      ...
      def parse_guide
        guide = @package.guide = EPUB3::Publication::Package::Guide.new
        @doc.xpath('/opf:package/opf:guide/opf:reference', EPUB3::NAMESPACES).each do |ref|
          reference = EPUB3::Publication::Package::Guide::Reference.new
      ...
      def parse_bindings
        bindings = @package.bindings = EPUB3::Publication::Package::Bindings.new
        @doc.xpath('/opf:package/opf:bindings/opf:mediaType', EPUB3::NAMESPACES).each do |elem|
          media_type = EPUB3::Publication::Package::Bindings::MediaType.new
      ...
      def extract_model(elem, id_map, xpath, klass=:DCMES, attributes=%w[id lang dir])
        models = elem.xpath(xpath, EPUB3::NAMESPACES).collect do |e|
          model = EPUB3::Publication::Package::Metadata.const_get(klass).new
